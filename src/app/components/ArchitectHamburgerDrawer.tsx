...
interface ArchitectHamburgerDrawerProps {
  open: boolean;
  role: string;
  userName?: string;
  onClose: () => void;
  onAction: (action: ArchMenuAction) => void;
}
...
const ARCHITECT_MENU: { key: ArchMenuAction; label: string }[] = [
  { key: "switchProfile",       label: "SWITCH PROFILE"         },
  { key: "blueprintsDocuments", label: "BLUEPRINTS & DOCUMENTS" },
  { key: "completedTasks",      label: "COMPLETED TASKS"        },
  { key: "invite",              label: "INVITE"                 },
  { key: "settings",            label: "SETTINGS"               },
];

export function ArchitectHamburgerDrawer({
  open,
  role,
  userName = "Arjun Singh",
  onClose,
  onAction,
}: ArchitectHamburgerDrawerProps) {
  const roleLabel = ROLE_LABELS[role] ?? role.toUpperCase();

  const handleItem = (key: ArchMenuAction) => {
    onClose();
              ...
              {ARCHITECT_MENU.map((item, i) => (
                <motion.button
                  ...
                  style={{
                    ...
                    borderBottom: i < ARCHITECT_MENU.length - 1 ? "1px solid #f3ede9" : "none",
                  }}
                  onMouseEnter={(e) => {
